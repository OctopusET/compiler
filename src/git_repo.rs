...
impl BareRepoWriter {
    pub fn create(output: &Path) -> Result<Self> {
        ...
        // Build into a temp bare repo first so partial runs never leave a broken output.git behind.
        let temp_output = {
        ...
        // The writer owns the repository, ODB, and mempack together, so extending
        // the borrow to the writer lifetime keeps the backend alive for the whole build.
        let mempack = {
            ...
            let mempack =
                unsafe { mem::transmute_copy::<Mempack<'_>, Mempack<'static>>(&*mempack) };
            AttachedMempack { odb, mempack }
        };
    ...
    pub fn commit_law(
        ...
        // Every law commit uses noon KST for stable, config-independent commit timestamps.
        let time = {
            ...
            GitTime::new(timestamp, KST_OFFSET_MINUTES)
        };
    ...
    pub fn commit_static(
        ...
        // Preserve the original repository authorship trailers on the bootstrap commit.
        let message = if INITIAL_COMMIT_CO_AUTHORS.is_empty() {
            message.to_owned()
        } else {
            let mut rendered = String::from(message.trim_end());
            rendered.push_str("\n\n");
            for (index, (name, email)) in INITIAL_COMMIT_CO_AUTHORS.iter().enumerate() {
                if index > 0 {
                    rendered.push('\n');
                }
    ...
    pub fn finish(self) -> Result<()> {
        if let Some(parent_commit) = self.parent_commit {
            // Flush the queued ODB writes into one pack before publishing refs.
            let mut pack = Buf::new();
            self.mempack.mempack.dump(&self.repo, &mut pack)?;

            let mut packwriter = self.mempack.odb.packwriter()?;
            packwriter.write_all(&pack)?;
            packwriter.commit()?;
            self.mempack.mempack.reset()?;
            self.repo
    ...
    fn commit_file(
        ...
        // Rebuild just the tree path that changed while keeping the rest of the tree shared.
        let tree_oid = {
            let base_tree = self
                .current_tree
...
mod tests {
    ...
    fn clamps_pre_epoch_dates() {
        ...
        assert_eq!(commit.time().seconds(), 12 * 60 * 60 - 9 * 60 * 60);
        assert_eq!(commit.time().offset_minutes(), 540);
    }
}
